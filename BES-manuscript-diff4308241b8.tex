...
%DIF DEL BES-manuscript-oldtmp-48267.tex   Tue Aug 26 12:14:04 2025
%DIF ADD BES-manuscript.tex                Tue Aug 26 12:12:29 2025
...
}\url{https://doi.org/10.5281/zenodo.16951117}\DIFadd{. This }\DIFaddend will be made
\DIFdelbegin \DIFdel{available via Zenodo prior to }\DIFdelend \DIFaddbegin \DIFadd{publicly available upon }\DIFaddend publication.
...
outputs, and manuscript-generating }\DIFaddend code for peer review \DIFdelbegin \DIFdel{is available here:
LINK
}\DIFdelend \DIFaddbegin \DIFadd{are provided as
supplemental files. An anonymous link for peer-review is here:
}\url{https://doi.org/10.5281/zenodo.16951117}\DIFadd{. This will be made
...
\item
  \(F_{110}\) \DIFdelbegin \DIFdel{, }\DIFdelend \DIFaddbegin \DIFadd{is a flux estimate across the two shallowest measurement
  layers.
}\item
  \DIFaddend \(F_{011}\) \DIFdelbegin \DIFdel{are fluxes }\DIFdelend \DIFaddbegin \DIFadd{is a flux estimate }\DIFaddend across the two \DIFdelbegin \DIFdel{most shallow layersand two deepest
  layers respectively.
The }\DIFdelend \DIFaddbegin \DIFadd{deepest measurement
  layers.
}\item
  \DIFadd{\(F_{101}\) is a flux estimate across the shallowest and deepest
  measurement layers.
}\end{itemize}

\DIFadd{For \(F_{110}\), \(F_{011}\), and \(F_{101}\), the }\DIFaddend diffusivity used in
...
flux depth. \DIFdelbegin %DIFDELCMD < \item
%DIFDELCMD <   %%%
\DIFdel{\(F_{101}\) is a surface flux estimate using
linear extrapolation
  using concentration measurements between the shallowest and deepest
  measurement layer. Hirano et al. (2003)and }\DIFdelend \DIFaddbegin \DIFadd{Uncertainty in all \(F_{ijk}\) values was quantified using
quadrature (Taylor, 2022). These computed fluxes could provide the basis
for additional soil flux estimates. For example, }\DIFaddend Tang et al. (2005)
\DIFdelbegin \DIFdel{used an
  approach similar to \(F_{101}\) in a temperate deciduous broadleaf
  forest and
ponderosa pine forest respectively.
}%DIFDELCMD < \end{itemize}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{estimated surface flux by linearly extrapolating \(F_{110}\) and
\(F_{011}\) to the soil surface.
}\DIFaddend 

\DIFdelbegin \DIFdel{Uncertainty in all \(F_{ijk}\) is computed through quadrature (Taylor,
2022).
}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend \subsection{Post processing evaluation}\label{sec-post-process}
...
Atmospheres}}%DIFAUXCMD
\DIFdel{, }\emph{\DIFdel{108}}%DIFAUXCMD
\DIFdel{(D20).
}%DIFDELCMD < \url{https://doi.org/10.1029/2003JD003766}
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend \bibitem[\citeproctext]{ref-jacksonEcologySoilCarbon2017}
