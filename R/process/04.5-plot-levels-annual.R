# Make a plot of the different methods and the fluxes across a whole year.
...
marshall_fluxes_small <- model_fluxes_marshall |> semi_join(field_data_joined,by=c("site","horizontalPosition"="sampling_location"))

mq_fluxes_small <- model_fluxes_mq |> semi_join(field_data_joined,by=c("site","horizontalPosition"="sampling_location"))
...
plot_daily_flux <- function(site_name,plot_legend,x_axis,y_axis) {
  ...
  base_plot <- ggplot() +
    geom_line(data = rbind(marshall_plot_data, mq_plot_data),
              aes(x = day, y = m_flux, color = method, linetype = method), linewidth = 1) +
    geom_point(data = field_data,
...
out_plots <- tibble(site = c("SJER","SRER","WREF","UNDE","KONZ","WOOD"),
...
out_big <- grid.arrange(g1,g2,g3,g4,g5,g6, layout_matrix=rbind(c(1,4),c(2,5),c(3,6)),
...
png('figures/flux-results-year.png', width = 7, height = 6, units = 'in', res = 300); plot(out_big); dev.off()
