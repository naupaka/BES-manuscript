...
### Purpose: Read in Licor JSON files and compute the fluxes. Converts the local time to UTC to avoid any time mismatches.
...
files_sources = list.files('R/functions',full.names = TRUE)
...
sites <- str_extract(file_names,pattern='(?<=Summer2024/)[:alpha:]{4}|(?<=_NEON_)[:alpha:]{4}') |> unique()
...
site_locations <- str_extract(file_names,pattern='(?<=Summer2024/)[:alpha:]{4}|(?<=_NEON_)[:alpha:]{4}')

# Map results to data frame
licor_flux_data = map2_df(.x=file_names,.y=site_locations,.f=~extract_licor_flux(.x,.y,site_ids)) |>
...
### Read in the automated instruments
...
automated_licor_site <- str_extract(automated_licor,pattern='(?<=Summer2024/)[:alpha:]{4}|(?<=_NEON_)[:alpha:]{4}')


# Map results to data frame
automated_flux_data <- map2_df(.x=automated_licor,.y=automated_licor_site,.f=~extract_licor_automated_flux(.x,.y,site_ids)) |>
...
licor_all_data <- licor_flux_data |>
  ...
  group_by(site) |>
  nest() |>
  inner_join(site_ids,by=c("site"="NEON")) |>
  select(site,data) |>
  unnest(cols=c(data)) |>
  ungroup()

save(licor_all_data,file='data/derived/licor-all-data.Rda')

### figure out when we visited each site with the LICOR with the appropriate time zone - yay!
measurement_times <- licor_all_data |>
  ...
  inner_join(site_ids,by=c("site"="NEON")) |>
  rename(curr_tz = tz) |>
  inner_join(sampling_locations,by=c("site"="NEON")) |>
  rename(sampling_location = location)

save(measurement_times,file='data/derived/field-data-info.Rda')
