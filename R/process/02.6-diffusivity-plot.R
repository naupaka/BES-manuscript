...
field_stats_data <- combined_data |>
  mutate(diffusivity_mq = map(.x=model_data_mq, .f=~(.x |> select(startDateTime,diffusivity) |> unnest(cols=c("diffusivity")) |> ungroup() |>
         ...
         diffusivity_field = map2(.x=neon_diffusivity_gradient,.y=combined_field,.f=~standardize_timestamps(.x,.y) |>
                                    ...
                                    mutate(method = "Field\nestimated") |> select(diffusivity_field,method) |>
