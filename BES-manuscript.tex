...
the field-measured fluxes, but the flux-gradient method that best
approximated field measurements varied by site.
...
We compared statistics of the modeled soil fluxes to field \(F_{S}\) at
each site (Table~\ref{tbl-r2-table}). As \texttt{neonSoilFlux} models
...
\caption{\label{tbl-r2-table}Reported statistics from the model fits.}
...
soil moisture data. SJER and WOOD required the largest number of
gap-filled measurements, which were primarily \(SWC\) and \(T_{S}\).
...
Here, values of SNR greater than unity indicates lower reported
uncertainty, as propogated by quadrature due to a relatively higher
...
modeled uncertainty \(\sigma_{ijk}\) decreases. As \(\epsilon\)
increases from left to right in each figure, the possible range of
values for each predicted flux value decreases and the proportion of
measured fluxes that fall within that range also decreases.
