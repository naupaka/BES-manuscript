...
We developed an R package (\texttt{neonSoilFlux}; LINK TO BE ADDED AFTER
PEER REVIEW) to compute half-hourly soil carbon fluxes and uncertainties
from NEON data. The objective of the \texttt{neonSoilFlux} package is a
...
R package \texttt{neonUtilities} (LINK TO BE ADDED AFTER PEER REVIEW).
...
\(F_{011} \leq F_{101} \leq F_{110} \leq F_{000}\) because the previous
flux estimates rely primarily on CO\(_2\) concentrations at deeper
depths, and could miss high concentrations of CO\(_{2}\) produced in
shallower layers.
