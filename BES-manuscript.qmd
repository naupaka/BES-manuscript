...
We compared statistics of the modeled soil fluxes to field $F_{S}$ at each site (@tbl-r2-table). As `neonSoilFlux` models half-hourly fluxes, we considered all measured fluxes over the duration of the half-hourly interval when computing the normalized root mean square error (NRMSE), $R^{2}$, at each of the flux methods ($F_{000}$, $F_{110}$, $F_{011}$, $F_{101}$) for the diffusivity models studied (Millington-Quirk or Marshall).
