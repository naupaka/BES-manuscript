...
Figure @gap-filled-stats reports which environmental measurements at each site relied on gap-filled measurements (left plot) and in the aggregate, the total number of gap-filled measurements used for each half-hourly flux computation.  the largest contribution to gap-filled data was soil water follwed by soil temperature.
...
Calculated diffusivity from both the `neonSoilFlux` method and back calculated from the LICOR are reported in Figure @diffusivity-plot. SAY MORE
