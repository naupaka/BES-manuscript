...
We developed an R package ([`neonSoilFlux`](https://CRAN.R-project.org/package=neonSoilFlux); @zobitzNeonSoilFluxComputeSoil2024) to compute half-hourly soil carbon fluxes and uncertainties from NEON data. The objective of the `neonSoilFlux` package is a unified workflow (@fig-package-diagram) for soil data acquisition and analysis that supplements the existing data acquisition R package (<https://CRAN.R-project.org/package=neonUtilities>; @lunchNeonUtilitiesUtilitiesWorking2025).
